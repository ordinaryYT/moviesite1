// server.js - FINAL FIXED: ONLY YOUR SUBTITLES LOAD IN VIDSRCS PLAYER (NOV 18 2025)
// Based on vidsrc API docs: Use vidsrc-embed.ru + sub_url + ds_lang + disable_subs=1
...
// FINAL EMBED - ONLY YOUR SUBTITLES LOAD (USING VIDSRCS 2025 API PARAMS)
app.get('/api/movies/:id/embed', authMiddleware, async (req, res) => {
  ...
  // FIXED: Use vidsrc-embed.ru (better custom sub support) + sub_url + ds_lang + disable_subs=1
  let url = `https://vidsrc-embed.ru/embed/${base}/${imdb_id}`;
  ...
  // Single track override - only your sub shows
  url += `?sub_url=${encodeURIComponent(subProxy)}&ds_lang=en&disable_subs=1`;
...
server.listen(PORT, () => {
  ...
  console.log(`YOUR CUSTOM SUBTITLES NOW LOAD IN VIDSRCS PLAYER - NO BUILT-IN SUBS`);
});
