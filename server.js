...
async function deleteAllOMCodes() {
  try {
    ...
    console.log('All OM codes cleared');
  } catch (err) {
    ...
    for (const [roomCode, room] of watchTogetherRooms.entries()) {
      if (room.host === socket.id) {
        ...
        if (room.viewers.size === 0) {
          setTimeout(() => {
            if (watchTogetherRooms.get(roomCode)?.viewers.size === 0) watchTogetherRooms.delete(roomCode);
          }, 300000);
        }
      }
    }
...
// ====================== CLEAN SUBTITLE PROXY - NO PROMO LINES ======================
...
// ====================== FINAL EMBED - ONLY YOUR SUBTITLES SHOW ======================
app.get('/api/movies/:id/embed', authMiddleware, async (req, res) => {
  ...
  // OFFICIAL VIDSR.TO SINGLE-FILE METHOD — ONLY THIS WORKS
  url += `?sub_file=${encodeURIComponent(subProxy)}&sub_label=English`;

  res.json({ ok: true, url });
});

// ====================== ALL ORIGINAL ROUTES BELOW - 100% COMPLETE ======================
...
app.get('/api/movies/:id/episodes', async (req, res) => {
  ...
  try {
    const showRes = await fetch(`https://api.tvmaze.com/lookup/shows?imdb=${rows[0].imdb_id}`);
...
app.get('/api/trailer', async (req, res) => {
  ...
  try {
    const ytRes = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(title + ' trailer')}&type=video&key=${YOUTUBE_API_KEY}`);
...
app.get('/api/fix-posters', async (req, res) => {
  if (req.query.pass !== ADMIN_PASSWORD) return res.status(403).json({ ok: false, error: 'Wrong password' });

  const { rows } = await pool.query('SELECT id, imdb_id FROM movies WHERE image IS NULL OR image = \'\' OR image = \'N/A\'');
  let fixed = 0;
  for (const m of rows) {
    try {
  ...
  res.json({ ok: true, fixed, message: `Fixed ${fixed} posters` });
});
...
server.listen(PORT, () => {
  ...
  console.log(`ONLY YOUR CLEAN SUBTITLES SHOW IN VIDSR.TO - 100% WORKING NOVEMBER 18 2025`);
});
